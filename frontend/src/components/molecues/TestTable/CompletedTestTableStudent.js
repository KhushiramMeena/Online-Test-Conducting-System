...
import { getTestResultStudent } from "../../../redux1/actions/studentTestAction";
          ...
          <TableBody>
            {this.props.testlist.map((test,index)=>(
              <TableRow key={index}>
                ...
                <TableCell><Button variant="outlined" onClick={(event)=>(this.onTestClick(event,test._id))}>View</Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
